...
from src.utils import (make_dia_nn_additional_param_commands,
                       make_dia_nn_var_mod_commands)
from src.validation import (validate_filepath_exists,
...
def main(config: Config, logger: logging.Logger):
    ...
    if config.library_free:
        ...
        for mods in mods_for_lib_creation:
            ...
            if predicted_library_path.exists():
                logger.info("Spectral library for mod(s) %s already exists.", mod_combination_str)
                continue
        ...
        if config.spectral_library_files_by_mod:
            ...
            for mods, library_df in spectral_library_df_by_mod.items():
                validate_spectral_library_not_empty(library_df, get_mod_combination_str(mods))
                library_path = (
                    result_path
    ...
    for mods, windows_for_mod in windows_by_mods.items():
        ...
        if mzml_path.exists() and spectrum_id_mapping_path.exists():
            logger.info("MzML with windows to search and ID mapping file already exist.")
        else:
            logger.info(
